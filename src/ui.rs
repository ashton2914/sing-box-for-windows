...
fn settings_card(ui: &mut egui::Ui, app: &mut App) {
    theme::card(ui, |ui| {
        ...
        ui.vertical(|ui| {
            ...
            ui.horizontal(|ui| {
                ui.label(
                    egui::RichText::new("Auto update interval (hours)")
        ...
        ui.vertical(|ui| {
            ui.horizontal(|ui| {
                ui.label(egui::RichText::new("Core").color(color::ON_SURFACE));
                let avail = ui.available_width();
                // Reserve room on the right for the two icon buttons.
                let combo_w = (avail - 2.0 * 32.0 - 2.0 * ui.spacing().item_spacing.x)
                    .max(120.0);
...
fn add_config_modal(ctx: &egui::Context, app: &mut App) {
    ...
    let result = theme::modal_dialog(
        ...
        |ui| {
            ...
            // ----- Name -----
            field_label(ui, "NAME");
            theme::input_singleline(ui, &mut app.add_dialog.name, "Config name");
            ...
            match app.add_dialog.kind {
                SourceKind::Remote => {
                    theme::input_singleline(ui, &mut app.add_dialog.url, "URL");
                }
                SourceKind::Local => {
                    ui.horizontal(|ui| {
                        let browse_w = 84.0;
